...
public:
	...
	virtual int32 GetTextureSlice() const override; // FB Bulgakov - Texture2D Array
	...
	virtual bool GetUseSpecifiedSlice() const override; // FB Bulgakov - Texture2D Array
...
protected:
	...
	void FillToolbar(FToolBarBuilder& ToolbarBuilder, const TSharedRef< FUICommandList > ToolkitCommands, TSharedRef<SWidget> LODControl, TSharedRef<SWidget> TextureSliceControl); // FB Bulgakov - Texture2D Array
	/**
	...
	/**
	 * FB Bulgakov - Gets the highest texture slice that this texture supports.
	 *
	 * @return Texture slice level.
	 */
	TOptional<int32> GetMaxTextureSlice() const;
...
private:
	...
	// Callback for changing the checked state of the TextureSlice check box.
	void HandleTextureSliceCheckBoxCheckedStateChanged(ECheckBoxState InNewState);

	// Callback for getting the checked state of the TextureSlice check box.
	ECheckBoxState HandleTextureSliceCheckBoxIsChecked() const;

	// Callback for determining whether the TextureSlice check box is enabled.
	bool HandleTextureSliceCheckBoxIsEnabled() const;

	// Callback for changing the value of the TextureSlice entry box.
	void HandleTextureSliceEntryBoxChanged(int32 NewTextureSlice);

	// Callback for getting the value of the TextureSlice level entry box.
	TOptional<int32> HandleTextureSliceEntryBoxValue() const;

	// Callback for clicking the TextureSliceMinus button.
	FReply HandleTextureSliceMinusButtonClicked();

	// Callback for clicking the TextureSlicePlus button.
	FReply HandleTextureSlicePlusButtonClicked();
	// FB Bulgakov End
...
private:
	...
	// FB Bulgakov Begin - Texture2D Array
	/** Which texture slice should be shown */
	int32 SpecifiedTextureSlice;
	/* When true, the specified texture slice value is used. */
	bool bUseSpecifiedTextureSlice;
	// FB Bulgakov End
