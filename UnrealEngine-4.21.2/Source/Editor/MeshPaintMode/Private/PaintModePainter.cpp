...
void FPaintModePainter::ActorSelected(AActor* Actor)
{
	...
	else if (PaintSettings->PaintMode == EPaintMode::Vertices)
	{
		...
		if (PaintSettings->VertexPaintSettings.MeshPaintMode == EMeshPaintMode::PaintNumbers)
		{
			bUpdateTextureArrayPalette = true;
		}
		// FB Bulgakov End
	}
...
bool FPaintModePainter::PaintInternal(const FVector& InCameraOrigin, const FVector& InRayOrigin, const FVector& InRayDirection, EMeshPaintAction PaintAction, float PaintStrength)
{
	...
	if (HoveredComponents.Num() > 0)
	{
		...
		FMeshPaintParameters Params;
		{
			...
			Params.NumberToPaint = PaintSettings->VertexPaintSettings.NumberToPaint; // FB Bulgakov - Texture2D Array
		...
		for (UMeshComponent* HoveredComponent : HoveredComponents)
		{
			...
			else if (PaintSettings->PaintMode == EPaintMode::Textures&& MeshAdapter->SupportsTexturePaint())
			{
				...
				if (TargetTexture2D)
				{
					...
					bPaintApplied |= MeshPaintHelpers::ApplyPerTrianglePaintAction(MeshAdapter, InCameraOrigin, BestTraceResult.Location, BestTraceResult.Normal, BrushSettings, FPerTrianglePaintAction::CreateRaw(this, &FPaintModePainter::GatherTextureTriangles, &TrianglePaintInfoArray, &MaterialSections, PaintSettings->TexturePaintSettings.UVChannel)); // FB Bulgakov - Texture2D Array
...
void FPaintModePainter::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	...
	// FB Bulgakov Begin - Texture2D Array
	if (bUpdateTextureArrayPalette ||
		T2DAPreviewCache != PaintSettings->VertexPaintSettings.Texture2DArray)
	{
		T2DAPreviewCache = PaintSettings->VertexPaintSettings.Texture2DArray;
		bUpdateTextureArrayPalette = false;
		if (Widget)
			Widget->RefreshTextureArrayPalette();
	}
	// FB Bulgakov End
