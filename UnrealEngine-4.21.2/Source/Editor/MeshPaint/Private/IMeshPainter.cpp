...
void IMeshPainter::RenderInteractorWidget(const FVector& InCameraOrigin, const FVector& InRayOrigin, const FVector& InRayDirection, FPrimitiveDrawInterface* PDI, EMeshPaintAction PaintAction, bool bRenderVertices, ESceneDepthPriorityGroup DepthGroup /*= SDPG_World*/)
{
	...
	if (HitResult.Component != nullptr)
	{
		...
		const float PointDrawSize = GetPainterSettings()->VertexPreviewSize; // FB Bulgakov - Texture2D Array
		...
		if (PDI != NULL)
		{
			...
			if (MeshAdapter->IsValid() && bRenderVertices && MeshAdapter->SupportsVertexPaint())
			{
				...
				const TArray<FVector>& InRangeVertices = MeshAdapter->SphereIntersectVertices(ComponentSpaceSquaredBrushRadius, ComponentSpaceBrushPosition, ComponentSpaceBrushNormal, ComponentSpaceCameraPosition, GetBrushSettings()); // FB Bulgakov - Texture2D Array
				...
				if (BrushSettings->bOnlySelectedTriangle && InRangeVertices.Num() == 3)
				{
					FVector p0 = ComponentToWorldMatrix.TransformPosition(InRangeVertices[0]) + HitResult.Normal * VisualBiasDistance;
					FVector p1 = ComponentToWorldMatrix.TransformPosition(InRangeVertices[1]) + HitResult.Normal * VisualBiasDistance;
					FVector p2 = ComponentToWorldMatrix.TransformPosition(InRangeVertices[2]) + HitResult.Normal * VisualBiasDistance;

					PDI->DrawLine(p0, p1, NormalLineColor, DepthGroup, DrawThickness);
					PDI->DrawLine(p2, p1, NormalLineColor, DepthGroup, DrawThickness);
					PDI->DrawLine(p2, p0, NormalLineColor, DepthGroup, DrawThickness);
				}
				// FB Bulgakov End
			}
		}
