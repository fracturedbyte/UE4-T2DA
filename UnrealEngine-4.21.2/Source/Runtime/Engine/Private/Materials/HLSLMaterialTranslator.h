...
protected:
	...
	const TCHAR* DescribeType(EMaterialValueType Type) const
	{
		...
		case MCT_Texture2DArray:return TEXT("texture2Darray"); // FB Bulgakov - Texture2D Array
	...
	int32 AccessUniformExpression(int32 Index)
	{
		...
		else if(CodeChunk.Type & MCT_Texture)
		{
			...
			case MCT_Texture2DArray:
				TextureInputIndex = MaterialCompilationOutput.UniformExpressionSet.Uniform2DTextureArrayExpressions.AddUnique(TextureUniformExpression);
	...
	*/	
		...
		) override
	{
		...
		if (TextureType != MCT_Texture2D && TextureType != MCT_Texture2DArray && TextureType != MCT_TextureCube && TextureType != MCT_VolumeTexture  && TextureType != MCT_TextureExternal) // FB Bulgakov - Texture2D Array
		{
		...
		EMaterialValueType UVsType = (TextureType == MCT_TextureCube || TextureType == MCT_VolumeTexture || TextureType == MCT_Texture2DArray) ? MCT_Float3 : MCT_Float2; // FB Bulgakov - Texture2D Array
	...
	virtual int32 CustomExpression( class UMaterialExpressionCustom* Custom, TArray<int32>& CompiledInputs ) override
	{
		...
		for( int32 i = 0; i < CompiledInputs.Num(); i++ )
		{
			...
			CodeChunk += TEXT(",");
			CodeChunk += *ParamCode;
			if (ParamType == MCT_Texture2D || ParamType == MCT_Texture2DArray || ParamType == MCT_TextureCube || ParamType == MCT_TextureExternal || ParamType == MCT_VolumeTexture) // FB Bulgakov - Texture2D Array
			{
				CodeChunk += TEXT(",");
				CodeChunk += *ParamCode;
