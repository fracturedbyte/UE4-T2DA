...
bool FUniformExpressionSet::IsEmpty() const
{
	return UniformVectorExpressions.Num() == 0
		...
		&& Uniform2DTextureArrayExpressions.Num() == 0 // FB Bulgakov - Texture2D Array
...
bool FUniformExpressionSet::operator==(const FUniformExpressionSet& ReferenceSet) const
{
	if (UniformVectorExpressions.Num() != ReferenceSet.UniformVectorExpressions.Num()
		...
		|| Uniform2DTextureArrayExpressions.Num() != ReferenceSet.Uniform2DTextureArrayExpressions.Num() // FB Bulgakov - Texture2D Array
	...
	// FB Bulgakov Begin - Texture2D Array
	for (int32 i = 0; i < Uniform2DTextureArrayExpressions.Num(); i++)
	{
		if (!Uniform2DTextureArrayExpressions[i]->IsIdentical(ReferenceSet.Uniform2DTextureArrayExpressions[i]))
		{
			return false;
		}
	}
...
{
	// FB Bulgakov Begin - Texture2D Array
	return FString::Printf(TEXT("(%u vectors, %u scalars, %u 2d tex, %u 2d tex array, %u cube tex, %u 3d tex, %u external tex, %u collections)"),
		UniformVectorExpressions.Num(), 
		UniformScalarExpressions.Num(),
		Uniform2DTextureExpressions.Num(),
		Uniform2DTextureArrayExpressions.Num(),
...
void FUniformExpressionSet::CreateBufferStruct()
{
	...
	check(Uniform2DTextureArrayExpressions.Num() <= 128); // FB Bulgakov - Texture2D Array
	...
	{
		check((NextMemberOffset & 0x7) == 0);
		new(Members) FUniformBufferStruct::FMember(*Texture2DArrayNames[i], TEXT("Texture2DArray"), NextMemberOffset, UBMT_TEXTURE, EShaderPrecisionModifier::Float, 1, 1, 1, NULL);
		NextMemberOffset += 8;
		new(Members) FUniformBufferStruct::FMember(*Texture2DArraySamplerNames[i], TEXT("SamplerState"), NextMemberOffset, UBMT_SAMPLER, EShaderPrecisionModifier::Float, 1, 1, 1, NULL);
		NextMemberOffset += 8;
	}
	// FB Bulgakov End
...
FUniformBufferRHIRef FUniformExpressionSet::CreateUniformBuffer(const FMaterialRenderContext& MaterialRenderContext, FRHICommandList* CommandListIfLocalMode, struct FLocalUniformBuffer* OutLocalUniformBuffer) const
{
	...
	if (UniformBufferStruct->GetSize() > 0)
	{
		...
		check(UniformBufferStruct->GetLayout().Resources.Num() == Uniform2DTextureExpressions.Num() * 2 + Uniform2DTextureArrayExpressions.Num() * 2 + UniformCubeTextureExpressions.Num() * 2 + UniformVolumeTextureExpressions.Num() * 2 + UniformExternalTextureExpressions.Num() * 2 + 2); // FB Bulgakov - Texture2D Array
		...
		// FB Bulgakov Begin - Texture2D Array
		// Cache 2D texture array uniform expressions.
		for (int32 ExpressionIndex = 0; ExpressionIndex < Uniform2DTextureArrayExpressions.Num(); ExpressionIndex++)
		{
			const UTexture* Value;
			ESamplerSourceMode SourceMode;
			Uniform2DTextureArrayExpressions[ExpressionIndex]->GetTextureValue(MaterialRenderContext, MaterialRenderContext.Material, Value, SourceMode);

			if (Value)
			{
				// Pre-application validity checks (explicit ensures to avoid needless string allocation)
				const FMaterialUniformExpressionTextureParameter* TextureParameter = (Uniform2DTextureArrayExpressions[ExpressionIndex]->GetType() == &FMaterialUniformExpressionTextureParameter::StaticType) ?
					&static_cast<const FMaterialUniformExpressionTextureParameter&>(*Uniform2DTextureArrayExpressions[ExpressionIndex]) : nullptr;
