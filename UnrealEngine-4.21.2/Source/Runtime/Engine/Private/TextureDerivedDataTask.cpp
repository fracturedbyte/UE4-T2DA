...
void FTextureCacheDerivedDataWorker::FTextureSourceData::Init(UTexture& InTexture, const FTextureBuildSettings& InBuildSettings, bool bAllowAsyncLoading)
{
	...
	if (!InBuildSettings.bCubemap && !InBuildSettings.bVolume && !InBuildSettings.bTexture2DArray) // FB Bulgakov - Texture2D Array
	{
		NumSlices = 1;
	}
	...
	if (Compressor && TextureData.Mips.Num())
	{
		...
		if (Compressor->BuildTexture(TextureData.Mips, CompositeTextureData.Mips, BuildSettings, CompressedMips))
		{
			...
			for (int32 MipIndex = 0; MipIndex < MipCount; ++MipIndex)
			{
				...
				check(NewMip->SizeZ == 1 || BuildSettings.bVolume || BuildSettings.bTexture2DArray); // Only volume can have SizeZ != 1 // FB Bulgakov - Texture2D Array
				...
				if (MipIndex == 0)
				{
					...
					DerivedData->NumSlices = BuildSettings.bCubemap ? 6 : ((BuildSettings.bVolume || BuildSettings.bTexture2DArray) ? CompressedImage.SizeZ : 1); // FB Bulgakov - Texture2D Array
